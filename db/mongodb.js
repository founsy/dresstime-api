...
exports.insertDressing = function(dressing){
    var deferred = Q.defer();
    var dressingOld = exports.getDressing(dressing.userid);
	MongoClient.connect(url, function(err, db) {
        console.log(dressingOld);
        if (typeof dressingOld !== 'undefined' && typeof dressingOld.dressing !== 'undefined'){
            dressingOld.dressing.push(dressing.dressing.length);
            db.collection('dressing').update({_id: dressingOld._id}, dressingOld, deferred.makeNodeResolver());
        } else {
            db.collection('dressing').insert(dressing, deferred.makeNodeResolver());
        }
	});
	return deferred.promise;
};

exports.getDressing = function(username){
    var deferred = Q.defer();
	MongoClient.connect(url, function(err, db) { 
		console.log("connected " + username);
		db.collection('dressing').findOne({'userid': username}, function(err, one){
			console.log("request " + one );
			if (err) deferred.reject(err);
			else if (one === null){
				console.log("end request failed");
...
exports.getClotheFromDressing = function(username, clotheId){
    var deferred = Q.defer();
	MongoClient.connect(url, function(err, db) { 
		console.log("connected " + clotheId);
		db.collection('dressing').findOne({'userid': username}, function(err, one){
			console.log("request " + one );
			if (err) deferred.reject(err);
			else if (one === null){
				console.log("end request failed");
				deferred.reject("No dressing found");
        	} else {
                console.log("");
                for (var i = 0; i < one.dressing.length; i++){
                    if (clothe[i].clothe_id === clotheId) {
                        deferred.resolve(one);
                    }
                }
        		deferred.reject("No clothe found");
            }   
			console.log("end request");
		});
	});
